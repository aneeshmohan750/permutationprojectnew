...
def text_conversion(request):
    ...
    if request.method == 'POST':
        ...
        convert_text = request.POST['convert_text']
    if convert_text.find(' ') != -1:
        lists=convert_text.split(' ')
        perms=list(itertools.permutations(lists))
    else:    
        if len(convert_text) > 10:
            perms = [''.join(p) for p in permutations(convert_text[0:10])]
        else :
            perms = [''.join(p) for p in permutations(convert_text)]       
        
    return render_to_response('qa/index.html', {'perms': perms}, context)
